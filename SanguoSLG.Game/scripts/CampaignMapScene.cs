...
public sealed partial class CampaignMapScene : Node3D
{
    ...
    private bool _targetingTransportDeploy;
            ...
            for (var activeIndex = 0; activeIndex < orderedActives.Count; activeIndex++)
            {
                ...
                else if (caster is not null && skill.Code == "lightning")
                {
                    foreach (var target in turn.Units.Where(x => x.Field.Owner != caster.Field.Owner
                                 ...
                                 .OrderBy(x => x.Field.Position.Distance(caster.Field.Position)).ThenBy(x => x.Id.Value))
                        _animSkillEffects.Add((activeTime + 0.14, casterId.Value, target.Id.Value, skill));
                }
                else if (caster is not null && skill.Code == "confound")
                {
                    var target = turn.Units.Where(x => x.Field.Owner != caster.Field.Owner
                            && x.State.Statuses.Any(status => status.IsDaze))
                        .OrderBy(x => x.Field.Position.Distance(caster.Field.Position))
                        .ThenBy(x => x.Id.Value).FirstOrDefault();
    ...
    private void OpenSupplyHub(CityId city)
    {
        ...
        var mine = Enumerable.Range(0, _pendingSupplyDeploys.Count)
            ...
            .ToList();
        box.AddChild(MakeLabel($"예약된 보급부대 ({mine.Count})   — 타일을 눌러 선택", 14, GoldBright));
        ...
        foreach (var i in mine)
        {
            ...
            tv.AddChild(l1);
            var l2 = MakeLabel(leader, 11, Parchment);
            ...
            tile.GuiInput += e =>
            {
                if (e is InputEventMouseButton { Pressed: true, ButtonIndex: MouseButton.Left }) { _depSelectedUnit = idx; OpenSupplyHub(city); }
            };
            grid.AddChild(cell);
        }
        ...
        if (_depSelectedUnit >= 0 && _depSelectedUnit < _pendingSupplyDeploys.Count)
        {
            var sidx = _depSelectedUnit;
            var req = _pendingSupplyDeploys[sidx].Req;
            var leader = _state.Generals.FirstOrDefault(g => g.Id == req.Vanguard)?.Name ?? "-";
            var total = req.Lines.Sum(l => l.Troops);
            var target = req.Target is { } tg
                ? "→ " + (_state.Cities.FirstOrDefault(c => c.Position == tg)?.Name ?? $"({tg.Q},{tg.R})")
                : "목표 미지정";
            box.AddChild(GoldRule());
            box.AddChild(MakeLabel($"◈ 보급 {total}명 · 주장 {leader} · {target}", 13, GoldBright));
    ...
    private void OpenTransportCompose(CityId city)
    {
        ...
        void Refresh()
        {
            ...
            {
                preview.Text = $"⚠ 수송 병력은 최대 {DeployService.TransportMaxTroops}명입니다. 현재 {total}명";
                return;
            }
        ...
        provSpin.ValueChanged += v => { provSlider.SetValueNoSignal(v); Refresh(); };
        Refresh();
        ...
        save.Pressed += () =>
        {
            ...
            var lines = _transportDraft
                ...
                .Select(p => new TransportLine(p.Key, p.Value))
                .ToList();
            var troops = lines.Sum(l => l.Troops);
    ...
    private void OpenTransportHub(CityId city)
    {
        _depModalCity = city;
        if (_modalLayer is not null) { _modalLayer.QueueFree(); _modalLayer = null; }
        ...
        var titleRow = new HBoxContainer();
        box.AddChild(titleRow);
        var title = MakeLabel($"◈  수송 예약   《 {cityName} 》  ⠿", 19, Gold);
        title.SizeFlagsHorizontal = Control.SizeFlags.ExpandFill;
        title.MouseFilter = Control.MouseFilterEnum.Ignore;
        titleRow.AddChild(title);
        var close = MakeButton("✕");
        ...
        var mine = Enumerable.Range(0, _pendingTransportDeploys.Count)
            .Where(i => _pendingTransportDeploys[i].Req.City == city)
            .ToList();
        box.AddChild(MakeLabel($"예약된 수송부대 ({mine.Count})   — 타일을 눌러 선택", 14, GoldBright));
        if (!mine.Contains(_depSelectedUnit)) { _depSelectedUnit = -1; }
        ...
        foreach (var ti in mine)
        {
            ...
            l2.HorizontalAlignment = HorizontalAlignment.Center;
            tv.AddChild(l2);
            var l3 = MakeLabel($"{total}명", 10, Parchment);
            l3.HorizontalAlignment = HorizontalAlignment.Center;
            tv.AddChild(l3);
            ...
            xbtn.OffsetRight = -2;
            xbtn.OffsetBottom = 22;
            xbtn.Pressed += () => { _pendingTransportDeploys.RemoveAt(idx); OpenTransportHub(city); };
            cell.AddChild(xbtn);
            tile.GuiInput += e =>
            {
                if (e is InputEventMouseButton { Pressed: true, ButtonIndex: MouseButton.Left }) { _depSelectedUnit = idx; OpenTransportHub(city); }
            };
            grid.AddChild(cell);
        }
        ...
        addTile.GuiInput += e =>
        {
            if (e is InputEventMouseButton { Pressed: true, ButtonIndex: MouseButton.Left }) { OpenTransportCompose(city); }
        };
        grid.AddChild(addTile);

        if (_depSelectedUnit >= 0 && _depSelectedUnit < _pendingTransportDeploys.Count)
        {
            var sidx = _depSelectedUnit;
            ...
            var total = req.Lines.Sum(l => l.Troops);
            box.AddChild(GoldRule());
            box.AddChild(MakeLabel($"◈ {dest}행 · 수송 {total}명 · 주장 {leader} · 금 {req.Gold} · 군량 {req.Provisions}", 13, GoldBright));
            var actRow = new HBoxContainer();
            actRow.AddThemeConstantOverride("separation", 6);
            var targetBtn = MakeButton("목표 지정", accent: true);
            targetBtn.CustomMinimumSize = new Vector2(96, 32);
            targetBtn.Pressed += () => BeginTransportTargeting(sidx);
            actRow.AddChild(targetBtn);
            var edit = MakeButton("편성 수정", accent: true);
            edit.CustomMinimumSize = new Vector2(96, 32);
            edit.Pressed += () => OpenTransportCompose(city);
            actRow.AddChild(edit);
            var rm = MakeButton("삭제");
            rm.CustomMinimumSize = new Vector2(72, 32);
            rm.Pressed += () => { _pendingTransportDeploys.RemoveAt(sidx); _depSelectedUnit = -1; OpenTransportHub(city); };
            actRow.AddChild(rm);
            box.AddChild(actRow);
        }

        box.AddChild(MakeLabel("수송부대는 속도 2, 군량 소모 50%로 이동합니다. 공격·방어·점령은 하지 않습니다.", 11, Parchment));
        var contentH = box.GetCombinedMinimumSize().Y;
        scroll.CustomMinimumSize = new Vector2(mw, Mathf.Min(contentH, mh));
        CenterAndDrag(panel, titleRow, mw, mh, box);
    }
    ...
    {
        var level = _state.ResearchOf(city.Owner, FactionResearch.ArmyGroupCode);
    ...
    private int CurrentResearchCost(City city)
    {
        ...
        if (cmd.Param == "troop" && _modalParam == 1)
        {
    ...
    private static Color ClassColor(TroopClass c) => c switch
    {
        ...
        TroopClass.Defense => new Color(0.74f, 0.55f, 0.38f),
        _ => Gold,
    };
    ...
    private void AskExecute(CityId city, int cmdIndex, GeneralId general, int p, HexCoord? plot = null)
    {
        ...
        if (cmd.Kind == CommandKind.Research && cmd.Param == "troop")
        {
            ...
            else if (troopCode == FactionResearch.ArmyGroupCode)
            {
                var level = _state.ResearchOf(cityData.Owner, troopCode);
                var maxLevel = _cb.ArmyGroupResearchMaxLevel;
                var next = System.Math.Min(level + 1, maxLevel);
                var cost = level >= maxLevel ? 0 : CommandEfficiency.CommandTroopResearchCost(next);
                extra = "\n집단군 편성 연구"
                    + $"\nLv.{level} → Lv.{next}/{maxLevel}"
                    + (level >= maxLevel
                        ? "\n※ 이미 최대 단계입니다"
                        : $"\n집단군 상한 {CommandEfficiency.ArmyGroupDeployLimit(level, _cb):N0}명 → {CommandEfficiency.ArmyGroupDeployLimit(next, _cb):N0}명"
                            + $"\n비용 {cost}금"
                            + $"\n[소요 {days}일]")
                    + (active is null ? "" : $"\n※ 이미 연구가 진행 중입니다: {TroopName(active.TroopCode)} · 남은 {System.Math.Max(0, active.CompletionDay - _state.Day)}일")
