...
- [x] Step 2 — 카메라 프레이밍·컨트롤 🔍: Fov 55 타이트 프레이밍, 휠 줌 + 중클릭 팬(CameraController3D)
- [x] Step 3 — 도시·유닛 3D 🔍: 도시 금색 대좌 + 한글 Label3D, 유닛 3D 토큰(원통+구)
- [x] Step 4 — 3D 클릭 이동 🔍: 레이캐스트→지면→헥사(WorldToHex 라운드트립 0오차), A* 경로 트윈 이동
- [x] Step 5 — 조명·환경 폴리시 🔍: 따뜻한 태양광+부드러운 그림자(4096), 하늘 앰비언트, 안개, SSAO, MSAA 4x, 물 테두리 4겹 디오라마, 좌표 해시 기반 타일 회전
- [x] 강 + 다리 🔍: TerrainType.River/Bridge(데이터 주도), 이웃 연결 방향으로 직선/커브/끝 모델·회전 자동 선택, 다리는 직선 구간에 배치. 기준각 실측 보정(0°)
- [ ] Step 6 — 마무리 + 회고 (사용자 만족도 확인 후)
