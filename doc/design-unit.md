...
> **관찰**: `ThunderCart`(4번)는 세 목록 어디에도 없어 전부 1이다. 앞서 "`ThunderCart`와
> `Catapult`는 외형과 기타 내용이 다르다"고 하신 것의 실체가 이 사거리 차이로 보인다.
> 의도한 것이 맞는지만 확인해 주면 이 메모는 지운다.
...
- **`ThunderCart` / `Catapult`** — 외형이 다르고, 이후 전투 수치 등 다른 내용도 갈라진다.
  현재 같은 것은 지형·이동력뿐이다.
