...
> ⚠️ **`cavalry.glb`는 폐기 대상이다(2026-08-06 확인).** 가능성 검증용으로 만든 것이라
> 다음 문제를 전부 안고 있다. 병종 2번 `Cavalry` 작업은 개선이 아니라 **처음부터 다시 만드는 것**이다.
>
> - **전부 사각형이다.** 계획 2(반곡선)가 하나도 적용되지 않았다 — **개선이 가장 시급한 부분**
> - **3기 고정**으로 박혀 있다. 계획 1(1기짜리 + `TroopFormation` 조립)과 맞지 않는다
> - **피벗 버그**: `origin_shift`가 스케일 적용 전 좌표에 들어가 실제 이동량이 스케일 배수만큼
>   줄었다. 말 다리가 지면에서 0.057 떠 있고 스윙 축도 고관절이 아니다.
>   `infantry_common.py`에서 고친 방식(스케일을 메시에 먼저 굽기)을 그대로 쓰면 된다
> - **갤럽 모션이 코드에 없다.** `UnitController3D`가 보병 규약(`body`/`leg_l`/`arm_r`)으로
>   바뀌어서 기병용 모션을 따로 붙여야 한다
