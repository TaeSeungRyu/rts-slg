...
>
> **남은 것: 갤럽 모션.** `UnitController3D`가 보병 규약(`body`/`leg_l`/`arm_r`)만 알아서
> 기병 부위(`leg_fl`/`rider_arm_r`)에는 걸리지 않는다. 기병용 모션을 따로 붙여야 한다.
...
| 2 | 기병 | `Cavalry` | `cavalry` | 육지 | 3 | 3 | 1 / 1 / 1 | O | X | X | 말을 타고 칼을 들고 있다 |
