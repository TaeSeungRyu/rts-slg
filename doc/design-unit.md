...
| # | 병종(잠정) | 식별자 | JSON 키 | 지형 | 이동력 | 사거리 유닛/건물/성 | 기본작업 | 이동모션 | 공격모션 | 모습 |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | 도검병 | `Swordsman` | `swordsman` | 육지 | 3 | 1 / 1 / 1 | O | O | O | 칼과 방패를 들고 있다 |
| 2 | 기병 | `Cavalry` | `cavalry` | 육지 | 5 | 1 / 1 / 1 | X | X | X | 말을 타고 칼을 들고 있다 |
| 3 | 궁병 | `Archer` | `archer` | 육지 | 3 | **2** / 1 / 1 | X | X | X | 활을 들고 있다 |
| 4 | 벽력거 | `ThunderCart` | `thunder_cart` | 육지 | 2 | 1 / 1 / 1 | X | X | X | 벽력거를 끌고 있다. 사거리가 `Catapult`와 갈린다 |
| 5 | 투석기 | `Catapult` | `catapult` | 육지 | 2 | **3** / **2** / 1 | X | X | X | 투석기를 끌고 있다. 사거리가 `ThunderCart`와 갈린다 |
| 6 | 공성탑 | `SiegeTower` | `siege_tower` | 육지 | 2 | **3** / **2** / 1 | X | X | X | 바퀴 달린 탑을 끌고 있다 |
| 7 | 상병 | `WarElephant` | `war_elephant` | 육지 | 3 | 1 / 1 / 1 | X | X | X | 코끼리를 끌고 있다 |
| 8 | 소선 | `SmallBoat` | `small_boat` | 대하 | 4 | 1 / 1 / 1 | X | X | X | 중국식 작은 배 |
| 9 | 중선 | `MediumShip` | `medium_ship` | 대하 | 3 | 1 / 1 / 1 | X | X | X | 중국식 중간 배 |
| 10 | 대선 | `LargeShip` | `large_ship` | 대하 | 3 | 1 / 1 / 1 | X | X | X | 중국식 큰 배 |

## 특수 유닛

| # | 병종(잠정) | 식별자 | JSON 키 | 지형 | 이동력 | 사거리 유닛/건물/성 | 기본작업 | 이동모션 | 공격모션 | 모습 |
|---|---|---|---|---|---|---|---|---|---|---|
| 11 | 장창병 | `Pikeman` | `pikeman` | 육지 | 3 | 1 / 1 / 1 | X | X | X | 긴 창을 들고 있다 |
| 12 | 낫병 | `Scytheman` | `scytheman` | 육지 | 3 | 1 / 1 / 1 | X | X | X | 낫을 들고 있다 |
| 13 | 대방패병 | `Shieldbearer` | `shieldbearer` | 육지 | 2 | 1 / 1 / 1 | X | X | X | 큰 방패를 들고 있다 |
| 14 | 산악병 | `Axeman` | `axeman` | 육지+산악 | 3 / 산악 1 | 1 / 1 / 1 | X | X | X | 도끼와 방패를 들고 있다 |
| 15 | 창기병 | `Lancer` | `lancer` | 육지 | 4 | 1 / 1 / 1 | X | X | X | 말을 타고 큰 창을 들고 있다 |
| 16 | 궁기병 | `HorseArcher` | `horse_archer` | 육지 | 5 | **2** / 1 / 1 | X | X | X | 작은 말을 타고 활을 들고 있다 |
| 17 | 대궁병 | `GreatBow` | `great_bow` | 육지 | 2 | **3** / **2** / 1 | X | X | X | 정말 큰 활을 들고 있다 |
| 18 | 판옥선 | `Panokseon` | `panokseon` | 대하 | 3 | 1 / 1 / 1 | X | X | X | 판옥선 모양의 배 |
| 19 | 거북선 | `Turtleship` | `turtleship` | 대하 | 3 | 1 / 1 / 1 | X | X | X | 거북선 모양의 배 |
...
인접해야 공격한다 — 이것이 `Catapult`(유닛 3 / 건물 2)와 갈리는 지점이다. 둘은 지형·이동력이
...
- **`ThunderCart` / `Catapult`** — 외형이 다르고, 사거리가 갈린다(`ThunderCart` 전부 1 /
  `Catapult` 유닛 3·건물 2). 지형·이동력만 같다. 이후 전투 수치에서도 더 갈라질 수 있다.
...
- 특기(`Skill`) 목록 — 첫 후보는 **기병 관통**(적을 통과하되 이동력 1 소모).
  이동 기본 규칙에서 뺀 것이라 [design-movement.md](./design-movement.md) 참조
