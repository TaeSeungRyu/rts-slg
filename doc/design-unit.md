...
>
> **갤럽 모션도 붙였다(2026-08-06).** `UnitController3D`가 `leg_fl` 유무로 규약을 판별하고,
> 다리를 "같은 위상 / 반대 위상" 묶음으로 다뤄 보병 행군과 기병 갤럽이 같은 코드를 쓴다.
> 남은 것은 기병 공격모션이다.
...
| 2 | 기병 | `Cavalry` | `cavalry` | 육지 | 3 | 3 | 1 / 1 / 1 | O | O | X | 말을 타고 칼을 들고 있다 |
