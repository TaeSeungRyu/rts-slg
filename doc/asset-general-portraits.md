...
정의된 장수의 얼굴을 공용 아이콘 하나(`icon_officer`) 대신 **장수별 초상**으로 교체하는 계획과
현행 배선 상태를 함께 기록한다. **현재 아이콘 세트의 테마(삼국지·금테·주홍·다크·회화 relief)를 유지**한다.

- 상태(2026-09-16): **초상 배선 완료**. `SanguoSLG.Game/assets/portraits/{id}.png`가 있으면 해당 장수 초상을 사용하고, 없으면 기존 공용 장수 아이콘으로 폴백한다. 장수 에디터는 같은 이미지에서 원형 얼굴 중심/확대 메타데이터를 저장하며, 좌상단 HUD·부대 정보 등 원형 얼굴 UI는 이 메타데이터를 사용한다.
...
- **배선**: `GeneralId → res://assets/portraits/{id}.png` 로더 연결 완료.
  ...
  - 적용 위치: 명령 모달 **장수 카드**, 정보 카드 **주둔/부대 지휘관**, 컨펌창, 장수 상세, 개발용 장수 에디터.
  - 원형 얼굴은 `data/general-portraits.json`의 `face_center_x`, `face_center_y`, `face_zoom`을 사용해 런타임에서 자른다.
  - 좌상단 HUD의 군주 초상도 일반 초상이 아니라 원형 얼굴 메타데이터를 사용한다.
...
## 6.5 향후 TODO (2026-08-20 논의 — 미착수)

초상 다양성·통일감을 위한 개선 항목. 기본 초상 배선은 완료되었고, 아래는 추가 품질 개선 항목이다.

- [ ] **지역별 복식 매핑 추가**(§3 속성표 확장) — 명단이 다국적(위촉오 + 고구려/백제/신라 + 일본 전국)이라 지역 복식이 가장 강한 구분 레버:
  - 중국(삼국): 흑칠 판갑 + 관모/투구
  - 삼국(한국): 두정갑·조우관(깃털관)·환두대도 분위기
  - 일본 전국: 大鎧 사무라이 갑옷 + 兜(투구)/전립
- [ ] **다양성 vs 통일감 손잡이 = Image Prompt weight** — 0.4~0.5 권장(높이면 앵커 얼굴로
      수렴해 다 비슷, 낮추면 인물 갈림). 화풍은 유지되며 얼굴만 변주되는 지점.
- [ ] **주의: per-캐릭터로 실제 장수 초상을 레퍼런스로 쓰지 말 것** — 얼굴까지 끌려와
      다양성이 죽고 화풍이 제각각(타 게임 그림 베끼기 문제). 앵커는 1장(icon_officer)만,
      얼굴은 프롬프트 텍스트 + seed로.
- [ ] **per-general 프롬프트 시트** 생성 — 속성표(무/지/나이/유형) + 지역 복식 반영
      Positive/Negative + seed 정책 한 줄씩. Fooocus 복붙용. (아키타입 20종 먼저 시험 후 확정)
