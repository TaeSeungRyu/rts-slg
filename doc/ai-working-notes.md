...
### 현재 작업 저장소
사용자가 `rts-slg-clean-probe`에서 잘 되는 것을 확인했으므로, 새 작업은 기본적으로
`D:\LOCAL-WORK-STATION\rts-slg-clean-probe`에서 진행한다. 다른 경로(`rts-slg`)가 현재 터미널 cwd로 잡혀 있어도,
사용자가 별도 지시하지 않는 한 clean-probe를 기준으로 상태 확인·수정·커밋·푸시한다.

### 요구사항별 QA는 필수다 (2026-09-11)
사용자가 기능 구현·버그 수정·UI 변경을 요청하면, 완료 보고와 커밋·푸시 전에 해당 요구사항별 QA를 반드시 수행한다.
- Core 로직 변경: 관련 xUnit 테스트를 추가/수정하고 통과시킨다.
- 영향 범위가 넓거나 회귀 위험이 있으면 전체 `dotnet test`를 실행한다.
- Game/Godot C# 변경: `dotnet build SanguoSLG.Game/SanguoSLG.Game.csproj`를 실행한다.
- 씬·리소스·UI·연출 변경: 가능한 경우 Godot headless 로딩/빌드까지 확인한다.
- 자동 검증이 어려운 시각·손맛 항목은 “내가 확인한 QA”와 “사용자가 실기기에서 확인할 항목”을 분리해 보고한다.
